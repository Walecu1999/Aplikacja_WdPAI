...
class Plansza(object):
    ...
    def __init__(self, wysokosc, szerokosc, ekran):
        ...
        self.gracze = ['C', 'B']
        self.pola_puste = [[Pionek(x, y, '-', self.ekran) for x in range((y + 1) % 2, self.KOLUMNY, 2)] for y in range(2, 6)]
        self.pola_pustych = list(chain.from_iterable(self.pola_puste))
        self.pola_czarne = [[Pionek(x, y, 'C', self.ekran) for x in range((y + 1) % 2, self.KOLUMNY, 2)] for y in range(0, 2)]
        self.pola_czarnych = list(chain.from_iterable(self.pola_czarne))
        self.pola_biale = [[Pionek(x, y, 'B', self.ekran) for x in range((y + 1) % 2, self.KOLUMNY, 2)] for y in range(6, 8)]
    ...
    def czy_jest_bicie_pionkiem(self, pionek):
        ...
        if pionek.kolor == 'B':
            if pionek.wsp_x + 2 < self.WIERSZE and pionek.wsp_y - 2 >= 0:
    ...
    def dodaj_bialy_pionek(self, pionek):
        ...
        wymiar_pola = self.szerokosc / self.WIERSZE
        ...
        for i in range(0, self.WIERSZE):
            for j in range(0, self.KOLUMNY):
                if (kolor_pola + kolor_pola2) % 2:
                    ...
                    pygame.draw.rect(self.ekran, self.CZARNE_POLE, pygame.Rect(wsp_x, wsp_y, self.szerokosc / self.WIERSZE, self.wysokosc / self.WIERSZE))
                else:
                    kolor_pola = kolor_pola+1
                    pygame.draw.rect(self.ekran, self.BIALE_POLE, pygame.Rect(wsp_x, wsp_y, self.szerokosc / self.WIERSZE, self.wysokosc / self.WIERSZE))
                wsp_x += self.szerokosc / self.WIERSZE
            wsp_y += self.wysokosc/self.WIERSZE
            wsp_x = 0
            kolor_pola2 += 1
            ...
            try:
                if pionek.kolor == 'C' and pionek.damka == False:
                    ...
                    pygame.draw.rect(self.ekran, self.CZARNE_POLE,
                                         pygame.Rect(wsp_x, wsp_y, self.szerokosc / self.WIERSZE, self.wysokosc / self.WIERSZE))
    ...
    def podswietlaj(self, pionek):
        ...
        pygame.draw.rect(self.ekran, self.PODSWIETLONE_POLE,
                         pygame.Rect((pionek.wsp_x ) * wymiar_pola, (pionek.wsp_y ) * wymiar_pola,
                                     self.szerokosc / self.WIERSZE,
                                     self.wysokosc / self.WIERSZE))
        if pionek.kolor == 'C' and pionek.damka == False:
            self.ekran.blit(CZARNY, (pionek.wsp_x * wymiar_pola - 360, pionek.wsp_y * wymiar_pola - 360))
        elif pionek.kolor == 'C' and pionek.damka == True:
            self.ekran.blit(CZARNA_DAMKA, (pionek.wsp_x * wymiar_pola - 360, pionek.wsp_y * wymiar_pola - 360))
    ...
    def przesuwaj(self, pionek):
        ...
        if pionek.kolor == 'C':
            self.ekran.blit(CZARNY, (pionek.wsp_x * wymiar_pola - 360, pionek.wsp_y * wymiar_pola - 360))
            pygame.draw.rect(self.ekran, self.CZARNE_POLE,
                             pygame.Rect((pionek.wsp_x+1) * wymiar_pola, (pionek.wsp_y+1) * wymiar_pola, self.szerokosc / self.WIERSZE,
                                         self.wysokosc / self.WIERSZE))
        elif pionek.kolor == 'B':
            self.ekran.blit(BIALY, (pionek.wsp_x * wymiar_pola - 360 , pionek.wsp_y * wymiar_pola - 360 ))
            pygame.draw.rect(self.ekran, self.CZARNE_POLE,
                             pygame.Rect((pionek.wsp_x+1) * wymiar_pola, (pionek.wsp_y+1)*wymiar_pola, self.szerokosc / self.WIERSZE, self.wysokosc / self.WIERSZE))
