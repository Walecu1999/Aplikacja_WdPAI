...
# Ekran gry
    SZEROKOSC = 1400
    WYSOKOSC = 800
    SZEROKOSC_PLANSZY = 640
    WYSOKOSC_PLANSZY = 640
...
# WYPISYWANIE WIADOMOSCI NA EKRAN
    ...
    def button(msg, x, y, w, h, ic, ac, action=None):
        ...
        if x + w > mouse[0] > x and y + h > mouse[1] > y:
            pygame.draw.rect(PLANSZOWKA.ekran, ac, (x, y, w, h))

            if click[0] == 1 and action is not None:
                pygame.time.wait(100)
                start()
        else:
            pygame.draw.rect(PLANSZOWKA.ekran, ic, (x, y, w, h))

        smallText = pygame.font.SysFont("timesnewroman", 20)
        textSurf = smallText.render(msg, True, (0,0,0))
        textRect = textSurf.get_rect()
        textRect.center = ((x + (w // 2)), (y + (h // 2)))
        PLANSZOWKA.ekran.blit(textSurf, textRect)
    ...
    #PLANSZOWKA.rysuj_poczatek()
        for event in pygame.event.get():
            button("Restart", 1060, 40, 50, 100, (155, 155, 155), (0, 0, 0), 'start()')
